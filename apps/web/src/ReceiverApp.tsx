      ...
      <main className="player-shell">
        <video
          ...
          onError={() => {
            ...
            setIsLocallyPlaying(false);
            setIsLocallyBuffering(false);
            setPlayerError("This browser reported a playback error.");
        ...
        {overlayLabel && (
          <div className="player-status-overlay">
            <div className="player-status-card">
              <div className="player-status-kicker">ClawTV</div>
              <h1>{overlayLabel}</h1>
  ...
  return (
    <main className="splash-screen">
      ...
      <section className="splash-card">
        <div className="splash-kicker">ClawTV Receiver</div>
        <h1>{connected ? "Ready For Playback" : "Waiting For ClawTV"}</h1>
        <p>
          {connected
            ? "The web receiver is standing by for a queue from the ClawTV server."
            : "Connect the ClawTV server to start playback here."}
        </p>
        <div className="splash-status" data-connected={connected}>
          <span className="status-dot" />
          <span>{connected ? "Connected" : "Server offline"}</span>
        </div>
      </section>
    </main>
  );
...
function clearRefreshRecoveryState(setRefreshRecoveryUntil: (value: number) => void) {
...
function sendJsonBeacon(path: string, body: unknown) {
  const url = resolveApiUrl(path);
