...
async function buildConversationalReply(input: {
  ...
  if (input.voiceConfig.backend === "openai") {
    const openAiReply = await runOpenAiVoiceAgentTurn(input);
    if (openAiReply) {
      return openAiReply;
    }
...
async function runOpenAiVoiceAgentTurn(input: {
  transcript: string;
  playback: PlaybackSnapshot & { diagnostics?: PlaybackDiagnostics | null };
  voiceConfig: VoiceConfig;
}): Promise<VoiceDecision | null> {
  const apiKey = process.env.OPENAI_API_KEY?.trim();

  if (!apiKey) {
    console.warn("OpenAI voice agent is configured but OPENAI_API_KEY is missing.");
    return null;
  }

  const tools = buildOpenAiVoiceAgentTools();
  const maxToolRounds = parseBoundedInteger(process.env.CLAWTV_OPENAI_MAX_TOOL_ROUNDS, 1, 8, 4);
  const conversationInput: OpenAiResponseInputItem[] = [
    {
      role: "user",
  ...
  for (let round = 0; round <= maxToolRounds; round += 1) {
    const response = await createOpenAiVoiceAgentResponse({
      apiKey,
      instructions: buildOpenAiVoiceAgentInstructions(input),
      input: conversationInput,
      tools
    });
    const output = response.output ?? [];

    conversationInput.push(...output as unknown as OpenAiResponseInputItem[]);

    for (const item of output) {
      if (item.type !== "function_call" || item.name !== "finalize_voice_turn") {
        continue;
      }

      return normalizeVoiceDecision(parseToolArguments(item.arguments), extractOpenAiOutputText(response));
    }

    const localToolCalls = output.filter((item) =>
      item.type === "function_call" && item.name && item.name !== "finalize_voice_turn" && item.call_id
    );

    if (localToolCalls.length === 0) {
      const rawText = extractOpenAiOutputText(response);
      return rawText ? normalizeVoiceDecision(parseLooseJsonObject(rawText) ?? {
        replyText: rawText,
        commandName: "none",
        payload: {},
    ...
    for (const toolCall of localToolCalls) {
      conversationInput.push({
        ...
        output: JSON.stringify(executeClawTvVoiceAgentTool(
          toolCall.name ?? "",
  ...
  return {
    ok: true,
    replyText: "I need one more clue before I put something on. What did you mean?",
    commandName: "none",
    payload: {},
    expectsReply: true,
    rawReplyText: "OpenAI voice agent reached the local tool round limit before finalizing."
  };
}

async function createOpenAiVoiceAgentResponse(input: {
  apiKey: string;
  instructions: string;
  input: OpenAiResponseInputItem[];
  tools: Array<Record<string, unknown>>;
}): Promise<OpenAiResponsesPayload> {
  const model = process.env.CLAWTV_OPENAI_MODEL?.trim() || process.env.OPENAI_MODEL?.trim() || "gpt-5.4";
  const timeoutSeconds = parseBoundedInteger(process.env.CLAWTV_OPENAI_TIMEOUT_SECONDS, 5, 180, 45);
  const controller = new AbortController();
  const timeout = setTimeout(() => controller.abort(), timeoutSeconds * 1000);

  try {
    const response = await fetch(process.env.CLAWTV_OPENAI_RESPONSES_URL?.trim() || "https://api.openai.com/v1/responses", {
      method: "POST",
      headers: {
      ...
      body: JSON.stringify({
        ...
        max_output_tokens: parseBoundedInteger(process.env.CLAWTV_OPENAI_MAX_OUTPUT_TOKENS, 128, 4096, 1200)
      }),
    ...
    if (!response.ok) {
      throw new Error(`OpenAI Responses API returned HTTP ${response.status}: ${body.slice(0, 500)}`);
    }

    return JSON.parse(body) as OpenAiResponsesPayload;
  } finally {
    clearTimeout(timeout);
  }
}

function buildOpenAiVoiceAgentInstructions(input: {
  transcript: string;
  playback: PlaybackSnapshot & { diagnostics?: PlaybackDiagnostics | null };
  voiceConfig: VoiceConfig;
}): string {
  return [
    ...
    "You are an agentic media-control assistant. Reason first, then choose tools if they help.",
    "Do not assume ambiguous phrases are show titles. They may be people, actors, hosts, networks, memories, vague descriptions, aliases, or user shorthand.",
    "Use your own knowledge to form hypotheses. Use tools to verify local library availability, inspect playback state, find recommendations, or resolve uncertainty.",
    "Do not follow a fixed tool sequence. Choose tools only when your reasoning says they are useful.",
    "If a person/topic appears in search results but no clear show was requested, explain the likely ambiguity and ask one short follow-up question.",
    "If the user asks for the latest episode of a show, first be sure the phrase is actually a show or series. If it is a person or topic, search/verify before choosing a series.",
    "Never claim playback started unless you finalize with a playback command.",
    "Keep replyText warm, direct, playful, and concise for a TV voice UI.",
    "Do not mention OpenAI, OpenClaw, tools, prompts, JSON, transport, or implementation details.",
    "When ready, call finalize_voice_turn with the exact ClawTV voice decision. Do not send a normal text answer instead.",
    "Voice decision schema: replyText string, commandName none|play|play-latest|shuffle|pause|resume|next|stop, payload object, expectsReply boolean.",
    "For play by exact title, use commandName play with payload {\"title\":\"...\"} or {\"mediaItemId\":\"...\"} if a catalog tool returned the exact item.",
    "For latest episode of a confirmed series, use commandName play-latest with payload {\"series\":\"...\"}.",
    "For shuffling a confirmed show, use commandName shuffle with payload {\"show\":\"...\",\"limit\":12}.",
    "For shuffling a confirmed network/source, use commandName shuffle with payload {\"network\":\"...\",\"limit\":12}.",
    "For pause, resume, next, or stop, use that commandName with an empty payload.",
    `Current playback context: ${describePlaybackContextForPrompt(input.playback)}`,
    `User said: ${input.transcript}`
  ].join("\n");
}

function buildOpenAiVoiceAgentTools(): Array<Record<string, unknown>> {
  const tools: Array<Record<string, unknown>> = [
    {
      type: "function",
      name: "get_current_playback",
      description: "Inspect what is currently playing and how much is left.",
      parameters: {
        type: "object",
        properties: {},
        required: [],
    ...
    },
    {
      type: "function",
      name: "search_catalog",
      description: "Search the local Plex-backed ClawTV catalog by title, show title, or summary text.",
      parameters: {
        type: "object",
        properties: {
          query: {
            type: "string",
            description: "The semantic thing to search for, such as a show, person, episode title, topic, actor, host, or phrase."
          },
          mediaType: {
            type: "string",
            enum: ["any", "show", "season", "episode", "movie"]
          },
          limit: {
            type: "number"
          }
        },
        required: ["query", "mediaType", "limit"],
        additionalProperties: false
      },
      strict: true
    },
    {
      type: "function",
      name: "list_shows",
      description: "List locally available shows, optionally filtered by starting text.",
      parameters: {
        type: "object",
        properties: {
          startsWith: {
            type: "string",
            description: "Optional title prefix. Use an empty string for no prefix."
          },
          limit: {
            type: "number"
          }
        },
        required: ["startsWith", "limit"],
        additionalProperties: false
      },
      strict: true
    },
    {
      type: "function",
      name: "list_networks",
      description: "List TV networks/sources with local shows in the catalog.",
      parameters: {
        type: "object",
        properties: {
          limit: {
    ...
    {
      type: "function",
      name: "list_network_shows",
      description: "List local shows for a TV network/source.",
      parameters: {
        type: "object",
        properties: {
          network: {
    ...
    },
    {
      ...
      description: "Get local episode recommendations for a confirmed show.",
      parameters: {
        type: "object",
        properties: {
          show: {
          ...
          strategy: {
            type: "string",
            enum: ["default", "random", "highly-rated"]
          },
          limit: {
            type: "number"
          },
        ...
        required: ["show", "strategy", "limit", "unwatchedOnly"],
        additionalProperties: false
      },
      strict: true
    },
    {
      type: "function",
      name: "recently_added",
      description: "Inspect recently added local catalog items.",
      parameters: {
        type: "object",
        properties: {
          mediaType: {
    ...
    {
      type: "function",
      name: "finalize_voice_turn",
      description: "Finalize the ClawTV voice turn after reasoning and any useful tools.",
      parameters: {
        type: "object",
        properties: {
          replyText: {
          ...
          commandName: {
            type: "string",
            enum: ["none", "play", "play-latest", "shuffle", "pause", "resume", "next", "stop"]
          },
          payload: {
            type: "object",
        ...
        required: ["replyText", "commandName", "payload", "expectsReply"],
        additionalProperties: false
      },
      strict: false
    }
  ...
  if (process.env.CLAWTV_OPENAI_WEB_SEARCH?.trim() !== "0") {
    tools.push({
      type: "web_search"
    });
  }
...
function executeClawTvVoiceAgentTool(
  name: string,
  ...
  switch (name) {
    case "get_current_playback":
      return {
        ok: true,
        playback: summarizePlaybackForAgent(playback)
      };
    case "search_catalog": {
      const query = typeof args.query === "string" ? args.query.trim() : "";
      const mediaType = parseCatalogMediaType(typeof args.mediaType === "string" ? args.mediaType : "any");
      const limit = clampAgentLimit(args.limit, 12);
      const result = db.searchCatalog({
        query,
        mediaType,
        limit
      });
      ...
      return {
        ...
        items: result.items.map((item) => summarizeMediaItemForAgent(item, query))
      };
    }
    case "list_shows": {
      const startsWith = typeof args.startsWith === "string" ? args.startsWith.trim() : "";
      const result = db.listShows({
        startsWith: startsWith || null,
        limit: clampAgentLimit(args.limit, 25)
      });
      return {
        ok: true,
        shows: result.shows
    ...
    case "list_networks":
      return {
        ok: true,
        networks: db.listNetworks(clampAgentLimit(args.limit, 25)).networks
      };
    case "list_network_shows": {
      const network = typeof args.network === "string" ? args.network.trim() : "";
      const result = db.listNetworkShows(network, clampAgentLimit(args.limit, 20));
      return {
        ok: true,
    ...
    case "recommend_episodes": {
      const show = typeof args.show === "string" ? args.show.trim() : "";
      const strategy = parseRecommendationStrategy(typeof args.strategy === "string" ? args.strategy : "default");
      const result = db.recommendEpisodes({
        show,
        strategy,
        limit: clampAgentLimit(args.limit, 8),
        unwatchedOnly: typeof args.unwatchedOnly === "boolean" ? args.unwatchedOnly : undefined
      });
      return {
        ok: true,
        show: result.show,
        ...
        items: result.items.map((entry) => ({
          item: summarizeMediaItemForAgent(entry.item),
          reason: entry.reason
        }))
      };
    }
    case "recently_added": {
      const mediaType = parseCatalogMediaType(typeof args.mediaType === "string" ? args.mediaType : "any");
      const result = db.listRecentlyAdded({
        mediaType,
        limit: clampAgentLimit(args.limit, 12)
      });
      return {
        ok: true,
        mediaType: result.mediaType,
        items: result.items.map((item) => summarizeMediaItemForAgent(item))
      };
    }
    default:
      return {
        ok: false,
        error: `Unknown ClawTV voice tool: ${name}`
      };
  }
}

function normalizeVoiceDecision(input: Record<string, unknown>, rawReplyText?: string | null): VoiceDecision {
  const commandName = parseVoiceCommandName(input.commandName);
  const replyText = typeof input.replyText === "string" && input.replyText.trim()
    ? input.replyText.trim()
    : commandName === "none"
      ? "I need one more clue before I put something on. What did you mean?"
      : "Okay.";
  ...
  return {
    ...
    expectsReply: typeof input.expectsReply === "boolean"
      ? input.expectsReply
      : commandName === "none" && replyText.endsWith("?"),
    rawReplyText: rawReplyText ?? JSON.stringify(input)
  };
}

function parseToolArguments(input: string | undefined): Record<string, unknown> {
  if (!input) {
    return {};
  }

  return parseLooseJsonObject(input) ?? {};
}
...
function extractOpenAiOutputText(response: OpenAiResponsesPayload): string | null {
...
function summarizePlaybackForAgent(snapshot: PlaybackSnapshot): Record<string, unknown> {
  return {
    state: snapshot.playbackState,
    queueLength: snapshot.queueLength,
    currentQueuePosition: snapshot.currentQueuePosition,
    currentItem: snapshot.currentItem ? summarizeMediaItemForAgent(snapshot.currentItem) : null,
    remainingMs: snapshot.context?.remainingMs ?? null,
    remainingEpisodesInSeason: snapshot.context?.remainingEpisodesInSeason ?? null,
    remainingSeasonsInShow: snapshot.context?.remainingSeasonsInShow ?? null
  };
}

function summarizeMediaItemForAgent(item: MediaItemSummary, query?: string): Record<string, unknown> {
  return {
    id: item.id,
    title: item.title,
    mediaType: item.mediaType,
    ...
    originallyAvailableAt: item.originallyAvailableAt,
    year: item.year,
    watchState: (item.viewCount ?? 0) > 0 ? `watched ${item.viewCount} time(s)` : "unwatched",
    summarySnippet: item.summary ? buildSummarySnippet(item.summary, query) : null,
...
function buildSummarySnippet(summary: string, query?: string): string {
  const normalizedSummary = summary.replace(/\s+/gu, " ").trim();
  const normalizedQuery = query?.trim();

  if (!normalizedQuery) {
    return normalizedSummary.slice(0, 260);
  }

  const index = normalizedSummary.toLowerCase().indexOf(normalizedQuery.toLowerCase());
  ...
  const end = Math.min(normalizedSummary.length, index + normalizedQuery.length + 170);
  const prefix = start > 0 ? "..." : "";
  const suffix = end < normalizedSummary.length ? "..." : "";

  return `${prefix}${normalizedSummary.slice(start, end)}${suffix}`;
}

function clampAgentLimit(value: unknown, fallback: number): number {
...
function resolveVoiceBackend(): VoiceConfig["backend"] {
  ...
  if (configured === "mock" || configured === "openclaw" || configured === "openai") {
    return configured;
  }
